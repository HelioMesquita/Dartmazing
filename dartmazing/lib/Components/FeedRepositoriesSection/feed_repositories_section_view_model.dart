...
class FeedRepositoriesSectionViewModel {
  ...
  FeedRepositoriesSectionViewModel(
      {required this.repositories, required this.type});
  ...
  FeedRepositoriesGroupViewModel groupForIndex(int index) {
    ...
    final firstCell = repositories.asMap().containsKey(currentIndex)
        ? repositories[currentIndex]
        : null;
    final secondCell = repositories.asMap().containsKey(currentIndex + 1)
        ? repositories[currentIndex + 1]
        : null;
    final thirdCell = repositories.asMap().containsKey(currentIndex + 2)
        ? repositories[currentIndex + 2]
        : null;
    return FeedRepositoriesGroupViewModel(
      firstCell: firstCell,
