...
class FeedNewsCell extends StatelessWidget {
  ...
  FeedNewsCell({Key? key, required this.viewModel}) : super(key: key);
  ...
  Widget generateGridImage(BuildContext context) {
    return Container(
      height: 220,
      child: ClipRRect(borderRadius: borderRadius, child: generateTable()),
    );
  }
  ...
  List<Widget> generateRown(List<Repository> list) {
    return list
        .map((e) => Image.network(e.imageURL, height: 110, fit: BoxFit.cover))
        .toList();
  }
}
